...
const CONTENT_COLLECTION = process.env.FIREBASE_CONTENT_COLLECTION || 'site_content'
const CONTENT_DOCUMENT = process.env.FIREBASE_CONTENT_DOCUMENT || 'homepage'
...
const buildFinalContent = (merged) => ({
...
const getLocalSiteContent = async () => {
  try {
    const raw = await fs.readFile(CONTENT_PATH, 'utf8')
    const parsed = JSON.parse(raw)
    const merged = mergeSiteContent(parsed)
    return buildFinalContent(merged)
  } catch {
...
export const getSiteContent = async () => {
  if (isFirebaseAdminConfigured()) {
    try {
      ...
      const doc = await db.collection(CONTENT_COLLECTION).doc(CONTENT_DOCUMENT).get()

      if (!doc.exists) {
        return clone(defaultSiteContent)
      }

      const merged = mergeSiteContent(doc.data())
      return buildFinalContent(merged)
    } catch (error) {
      console.error('Firebase read failed, falling back to local content store.', error)
...
export const saveSiteContent = async (value) => {
  ...
  const finalContent = buildFinalContent(merged)

  if (isFirebaseAdminConfigured()) {
    try {
      const db = getFirestoreAdmin()
      await db.collection(CONTENT_COLLECTION).doc(CONTENT_DOCUMENT).set(finalContent, { merge: true })
      return finalContent
    } catch (error) {
      console.error('Firebase write failed, saving content locally instead.', error)
    }
  }
